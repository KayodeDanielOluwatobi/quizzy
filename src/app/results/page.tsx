...
function ResultsContent() {
  ...
  return (
    <div className={`w-full ${showReview ? 'max-w-4xl' : 'max-w-lg'} transition-all duration-500 rounded-xl bg-card border border-border p-5 md:p-10 flex flex-col items-center`}>
      ...
      <div className="flex flex-col items-center text-center mb-8 md:mb-10 w-full">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-foreground uppercase tracking-tight leading-none mb-1">
          Exam Report
        </h1>
      </div>
      ...
      <div className={`w-full flex ${showReview ? 'flex-col md:flex-row gap-8 items-start' : 'flex-col items-center'}`}>
        ...
        {/* FIX: Changed sticky to md:sticky md:top-6 so it un-sticks on mobile! */}
        <div className={`w-full ${showReview ? 'md:w-1/3 md:sticky md:top-6' : 'w-full'} flex flex-col items-center`}>
          <div className="w-full bg-muted/30 border border-border rounded-xl p-6 md:p-8 flex flex-col items-center mb-6">
            <p className="text-xs md:text-sm font-bold text-muted-foreground uppercase tracking-widest mb-4">Final Score for {name}</p>
            <div className="flex items-baseline gap-2 mb-2">
              <span className="text-6xl md:text-7xl font-black tabular-nums tracking-tighter text-foreground leading-none">{percentage}</span>
          ...
          <div className="w-full grid grid-cols-3 gap-2 md:gap-3 mb-8">
            <div className="flex flex-col items-center p-3 md:p-4 bg-card border border-border rounded-md">
              <CheckCircle2 size={18} className="text-green-500 mb-2" strokeWidth={2.5} />
              <span className="text-xl md:text-2xl font-black tabular-nums">{score}</span>
            ...
            <div className="flex flex-col items-center p-3 md:p-4 bg-card border border-border rounded-md">
              ...
              <span className="text-xl md:text-2xl font-black tabular-nums">{total}</span>
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Total</span>
            </div>
            <div className={`flex flex-col items-center justify-center p-3 md:p-4 border rounded-md ${accentColor}`}>
              <span className="text-2xl md:text-3xl font-black leading-none mt-1">{grade}</span>
        ...
        {showReview && (
          <div className="w-full md:w-2/3 h-125 md:h-175 overflow-y-auto pr-2 custom-scrollbar space-y-6 animate-in fade-in slide-in-from-right-4">
            <div className="sticky top-0 bg-card z-10 pb-4 border-b border-border mb-4">
              ...
              <p className="text-xs font-medium text-muted-foreground mt-1">Comprehensive review of your performance.</p>
            </div>
            ...
            {flatQuestions.map((q: any, idx: number) => {
              ...
              return (
                <div key={idx} className={`p-4 md:p-5 rounded-lg border transition-all ${containerStyle}`}>
                  ...
                  {/* --- MCQ RENDER --- */}
                  {!isMtf && (
                    <div className="space-y-3 bg-background/50 p-3 md:p-4 rounded-md border border-border/50">
                  ...
                  {isMtf && (
                    <div className="space-y-2 mt-2">
                      {q.options.map((stmt: string, i: number) => {
                        ...
                        return (
                          <div key={i} className={`p-2 md:p-3 rounded-md text-xs md:text-sm border flex flex-col sm:flex-row justify-between gap-3 ${isStmtCorrect ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                            <p className="font-medium flex-1 leading-snug">{String.fromCharCode(65 + i)}. {stmt}</p>
                            <div className="flex flex-col sm:items-end gap-1 shrink-0 bg-background/50 px-3 py-1.5 rounded border border-border/50">
                  ...
                  {q.explanation && (
                    <div className="mt-5 pt-4 border-t border-border/50">
                      <div className="flex items-center gap-2 mb-2 text-primary font-bold text-[10px] md:text-xs uppercase tracking-wider">
...
export default function ResultsPage() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-background text-foreground transition-colors duration-300 p-4 md:p-6 font-sans">
